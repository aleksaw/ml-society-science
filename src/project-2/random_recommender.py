...
class RandomRecommender:
    ...
    def _fit_models(self, quick = False):
        self.data_in_model = self.data.shape[0]
    ...
    def estimate_utility(self, data, actions, outcomes, policy=None):
        ...
        for train, test in rskf.split(data, outcomes, outcomes+2*actions):
            ...
            policy.fit_treatment_outcome(Xtr, Atr, Ytr, quick=True)
            A = np.array([policy.recommend(Xte.iloc[i:i+1,:]) for i in range(len(Xte))])
            # We only know the outcomei n the cases where our recommendation
            # equals the historical action
            mask = (Ate.values.ravel()==A)
    ...
    def recommend(self, user_data):
        ...
        return np.random.choice(np.arange(self.n_actions),
                                p=self.get_action_probabilities(user_data))

    # Observe the effect of an action. This is an opportunity for you
    # to refit your models, to take the new information into account.
    def observe(self, user, action, outcome):
        self.data = np.append(self.data, self.scaler.transform(user.reshape(1,-1)))
        ...
        if self.data.shape[0] > self.refit_trigger * self.data_in_model:
