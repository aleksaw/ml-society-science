...
class ThompsonContextBandit:
    ...
    def __init__(self, n_obs, n_actions, n_outcomes, alpha):
        self.n_actions = n_actions
        self.n_outcomes = n_outcomes
        self.n_obs = n_obs
        self.reward = self._default_reward
        self.average_reward = alpha + np.zeros(n_obs, self.n_actions)
        self.counts = alpha + np.zeros(n_obs, self.n_actions)
    ...
    def get_action_probabilities(self, user_data = None):
        return np.random.beta(self.alpha[user_data,:], self.beta[user_data,:])


    # Return recommendations for a specific user datum
    # This should be an integer in range(self.n_actions)
    def recommend(self, user_data=None):
        #model(fit(bootstrap(dataset)))
        #return model.get:best_action
